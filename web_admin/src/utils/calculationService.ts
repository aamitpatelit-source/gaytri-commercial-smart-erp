...
export function evaluateAttendanceStatus(
  ...
  const checkIn = new Date(checkInIso);
  const shiftStartMins = parseTimeToMinutes(settings.shift_start_time);
  const checkInMins = checkIn.getHours() * 60 + checkIn.getMinutes();

  // Late evaluation with grace period
  const isLate = checkInMins > (shiftStartMins + settings.late_grace_period);

  let isEarlyDeparture = false;
  if (checkOutIso) {
    const checkOut = new Date(checkOutIso);
    const shiftEndMins = parseTimeToMinutes(settings.shift_end_time);
    const checkOutMins = checkOut.getHours() * 60 + checkOut.getMinutes();
    isEarlyDeparture = checkOutMins < (shiftEndMins - settings.early_checkout_grace_period);
  }

  const workedHours = calculateWorkedHours(checkInIso, checkOutIso, settings);
