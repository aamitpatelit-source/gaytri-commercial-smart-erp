...
async function main() {
  ...
  const mgrRes = await query('SELECT id, password_hash, role FROM admins WHERE email = $1', [managerEmail]);
  ...
  const adminRes = await query("SELECT id, email, password_hash FROM admins WHERE role = 'ADMIN' OR role = 'SUPER_ADMIN' LIMIT 1");
  ...
  await query('UPDATE admins SET password_hash = $1 WHERE id = $2', [testHash, managerId]);
  await query('UPDATE admins SET password_hash = $1 WHERE id = $2', [testHash, adminId]);
  ...
  try {
    ...
    const employeesRes = await query('SELECT id, employee_id, full_name FROM employees WHERE is_active = TRUE LIMIT 3');
    ...
    const assignRes = await makeRequest(
      `${LOCAL_URL}/auth/managers/${managerId}/employees`,
      'POST',
    ...
    // Verify DB count
    const countRes = await query('SELECT COUNT(*)::int as count FROM manager_employees WHERE manager_id = $1', [managerId]);
    ...
    console.log('Roster employees:', rosterList.map((e: any) => `${e.full_name} (${e.employee_id})`));
    ...
    await query('DELETE FROM attendance WHERE employee_id = $1 AND date = CURRENT_DATE', [targetEmployee.id]);
    ...
    const historyRes = await makeRequest(
      `${LOCAL_URL}/attendance/history?start_date=${todayStr}&end_date=${todayStr}`,
      'GET',
      { 'Authorization': `Bearer ${managerToken}` },
      null
    );
    ...
    const matchedRecord = Array.isArray(records) ? records.find((r: any) => r.employee_id === targetEmployee.employee_id) : null;
    ...
    await query('DELETE FROM leave_requests WHERE employee_id = $1', [targetEmployee.id]);
    
    const leaveIdRes = await query(`
    ...
    const leavesRes = await makeRequest(
      `${LOCAL_URL}/leaves/requests`,
      'GET',
      { 'Authorization': `Bearer ${managerToken}` },
      null
    );
    ...
    const leaveReqFound = Array.isArray(leavesList) ? leavesList.find((r: any) => r.id === leaveId) : null;
    ...
    const approveRes = await makeRequest(
      `${LOCAL_URL}/leaves/requests/${leaveId}/approve`,
      'POST',
      { 'Authorization': `Bearer ${managerToken}` },
      { remarks: 'Approved via E2E test' }
    );
    console.log(`Approve POST Status: ${approveRes.status}`);
    console.log(`Approve POST Response: ${approveRes.body}`);

    // Verify DB state
    const leaveFinalRes = await query('SELECT status, approved_by FROM leave_requests WHERE id = $1', [leaveId]);
    console.log('Final DB Leave Status:', leaveFinalRes.rows[0]);

  } finally {
    // 14. Restore original password hashes
    await query('UPDATE admins SET password_hash = $1 WHERE id = $2', [managerOrigHash, managerId]);
    await query('UPDATE admins SET password_hash = $1 WHERE id = $2', [adminOrigHash, adminId]);
