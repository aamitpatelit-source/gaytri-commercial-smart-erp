...
import { markAttendance, getAttendanceHistory, employeeCheckOut } from '../controllers/attendanceController';
...
async function verifyFullManagerUIWorkflow() {
  ...
  const adminRes = await query(`SELECT id FROM admins WHERE role IN ('ADMIN', 'SUPER_ADMIN', 'MANAGER') LIMIT 1`);
  const managerId = adminRes.rows[0].id;
  const mockUser = { id: managerId, role: 'SUPER_ADMIN' };

  let empRes = await query(`SELECT id, employee_id, full_name FROM employees LIMIT 1`);
  if (empRes.rows.length === 0) {
    ...
    await query(
      `INSERT INTO employees (id, employee_id, full_name, email, mobile, joining_date, salary_type)
       VALUES ($1, 'EMP001', 'John Doe', 'john@example.com', '9876543210', CURRENT_DATE, 'MONTHLY')`,
      [newEmpId]
    );
    empRes = await query(`SELECT id, employee_id, full_name FROM employees LIMIT 1`);
  }
  ...
  await query(`DELETE FROM attendance WHERE employee_id = $1 AND date = $2`, [empUuid, today]);
  ...
  const dbRowRes = await query(
    `SELECT id, employee_id, date, status, time, check_in_time, check_out_time 
  ...
  const historyReq: any = {
    query: { start_date: today, end_date: today },
    user: mockUser
  };
  const historyRes = createMockRes();
  await getAttendanceHistory(historyReq, historyRes);

  const apiLogs = historyRes.data.logs;
  const targetLog = apiLogs.find((l: any) => l.employee_uuid === empUuid);
  ...
  const _originalStatuses: Record<string, string> = {};
  const _checkInTimes: Record<string, string | null> = {};
  const _checkOutTimes: Record<string, string | null> = {};
  const _localStatuses: Record<string, string> = {};
  ...
  const _checkingOutEmpIds = new Set<String>();
  ...
  const coReq: any = { body: { employee_id: empUuid }, user: mockUser, ip: '127.0.0.1' };
  const coRes = createMockRes();
  await employeeCheckOut(coReq, coRes);
  ...
  await employeeCheckOut(coReq, dupCoRes);
  ...
  await getAttendanceHistory(historyReq, postCoHistoryRes);
  const postCoLog = postCoHistoryRes.data.logs.find((l: any) => l.employee_uuid === empUuid);
  ...
  await markAttendance(markReq, modRes);
