...
async function main() {
  ...
  const empRes = await query('SELECT id, full_name, department_id FROM employees WHERE id = $1', [empId]);
  ...
  const mappings = await query('SELECT department_id FROM manager_departments WHERE manager_id = $1', [managerId]);
  ...
  const tempHash = bcrypt.hashSync('workforce@2026', 10);
  await query('UPDATE admins SET password_hash = $1 WHERE id = $2', [tempHash, managerId]);
  ...
  if (!deptIds.includes(employeeDeptId)) {
    ...
    const deptExists = await query('SELECT 1 FROM departments WHERE id = $1', [employeeDeptId]);
    if (deptExists.rows.length === 0) {
      await query('INSERT INTO departments (id, name) VALUES ($1, \'Temp Production\') ON CONFLICT DO NOTHING', [employeeDeptId]);
      console.log(`[Setup] Temporarily backfilled department ID ${employeeDeptId} in departments table.`);
    }

    await query('INSERT INTO manager_departments (manager_id, department_id) VALUES ($1, $2)', [managerId, employeeDeptId]);
  ...
  try {
    ...
    if (employees.length > 0) {
      const targetEmp = employees.find((e: any) => e.id === empId) || employees[0];
    ...
    await query('UPDATE admins SET password_hash = $1 WHERE id = $2', [originalHash, managerId]);
    console.log('\n[Cleanup] Restored original password hash in database.');

    // 10. Delete temporary department mapping
    if (tempMappingAdded) {
      await query('DELETE FROM manager_departments WHERE manager_id = $1 AND department_id = $2', [managerId, employeeDeptId]);
