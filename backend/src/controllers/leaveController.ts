  ...
  try {
    ...
    if (userRole === 'EMPLOYEE') {
      const balance = await query(
        `SELECT b.casual_leave, b.sick_leave, b.paid_leave,
  ...
  try {
    const result = await query(
      `UPDATE leave_balances 
       ...
       WHERE id = $4
       RETURNING *`,
      [casual_leave, sick_leave, paid_leave, id]
    );
...
export const getLeaveRequests = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (userRole === 'EMPLOYEE') {
      const result = await query(
        `SELECT r.id, r.start_date, r.end_date, r.type, r.reason, r.status, r.remarks, r.approved_at,
...
export const approveLeaveRequest = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const reqRes = await client.query(
      `SELECT r.id, r.employee_id, r.start_date, r.end_date, r.type, r.reason, r.status,
              e.department_id
       FROM leave_requests r
       JOIN employees e ON r.employee_id = e.id
    ...
    const hasPermission = await canManageEmployee(managerId!, leaveReq.employee_id, userRole!);
    if (!hasPermission) {
      await client.query('ROLLBACK');
      return res.status(403).json({ success: false, message: 'Forbidden. Employee is not assigned to you.' });
    }

    const start = moment(leaveReq.start_date);
    const end = moment(leaveReq.end_date);
    const duration = end.diff(start, 'days') + 1;

    // 3. Check and deduct balance if CL, SL, PL
    if (leaveReq.type !== 'UNPAID') {
      const balRes = await client.query(
        'SELECT id, casual_leave, sick_leave, paid_leave FROM leave_balances WHERE employee_id = $1 FOR UPDATE',
      ...
      await client.query(
        `UPDATE leave_balances SET ${updatedCol} = ${updatedCol} - $1 WHERE id = $2`,
        [duration, bal.id]
      );
    }
    ...
    while (curr.format('YYYY-MM-DD') <= endFormatted) {
      ...
      const attCheck = await client.query(
        'SELECT id, status, remarks FROM attendance WHERE employee_id = $1 AND date = $2 FOR UPDATE',
      ...
      if (attCheck.rows.length === 0) {
        ...
        await client.query(
          `INSERT INTO attendance (employee_id, manager_id, date, time, status, remarks, source)
           VALUES ($1, $2, $3, $4, 'LEAVE', $5, 'MANAGER_MANUAL')`,
          [leaveReq.employee_id, managerId, dateStr, timeStr, attRemarks]
        );
      } else {
        ...
        if (attRow.status !== 'LEAVE') {
          // Update status
          await client.query(
            `UPDATE attendance SET status = 'LEAVE', remarks = $1, manager_id = $2 WHERE id = $3`,
            [attRemarks, managerId, attRow.id]
          );

          // Write audit log entry
          await client.query(
            `INSERT INTO attendance_audit_logs (attendance_id, changed_by, old_status, new_status, old_remarks, new_remarks, reason)
             VALUES ($1, $2, $3, 'LEAVE', $4, $5, 'Automatic leave marking on approval')`,
            [attRow.id, managerId, attRow.status, attRow.remarks, attRemarks]
          );
        }
      }
...
export const rejectLeaveRequest = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const reqRes = await client.query(
      `SELECT r.id, r.employee_id, r.status, e.department_id
       FROM leave_requests r
       JOIN employees e ON r.employee_id = e.id
       WHERE r.id = $1 FOR UPDATE`,
      [id]
