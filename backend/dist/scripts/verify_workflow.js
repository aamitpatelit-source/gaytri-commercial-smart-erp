...
async function runTests() {
    ...
    try {
        ...
        const settingsRes = await (0, db_1.query)('SELECT timezone, business_hours_end FROM company_settings LIMIT 1');
        ...
        const empRes = await (0, db_1.query)('SELECT id, full_name, joining_date FROM employees WHERE is_active = TRUE LIMIT 1');
        if (empRes.rows.length === 0) {
            throw new Error('No active employees found in the DB. Please seed the DB first.');
        }
        const testEmp = empRes.rows[0];
        console.log(`Using test employee: ${testEmp.full_name} (ID: ${testEmp.id}, Joining Date: ${(0, moment_timezone_1.default)(testEmp.joining_date).format('YYYY-MM-DD')})`);
        // Clean up any pre-existing records for this employee on testDate
        await (0, db_1.query)('DELETE FROM attendance WHERE date = $1', [testDate]);
        // --- TEST 1: Check-in only (no checkout) ---
        console.log('\n--- TEST 1: Check-in only (no checkout) ---');
        const dbCheckIn = '09:05:00';
        const dbCheckOut = null;
        const resolvedCheckOut1 = (0, attendanceUtils_1.getEffectiveCheckOut)(dbCheckOut, dbCheckIn, settings);
        ...
        const resolvedCheckOut2 = (0, attendanceUtils_1.getEffectiveCheckOut)(dbCheckOut2, dbCheckIn2, settings);
        ...
        const resolvedCheckOut3 = (0, attendanceUtils_1.getEffectiveCheckOut)(dbCheckOut3, dbCheckIn3, settings);
        ...
        await (0, db_1.query)(`INSERT INTO attendance (employee_id, date, time, check_in_time, status, source)
       VALUES ($1, $2, '09:05:00', '09:05:00', 'LATE', 'MANAGER_MANUAL')`, [emp1, testDate]);
        ...
        const tempEmpRes = await (0, db_1.query)(`INSERT INTO employees (employee_id, full_name, department_id, designation_id, shift_id, password_hash, joining_date, is_active, mobile, salary_type, email)
       VALUES ('GC-TEMP-TEST', 'Future Employee', 1, 1, 1, 'mock_hash', $1, TRUE, '1234567890', 'MONTHLY', 'temp_future@example.com')
       RETURNING id`, [futureDate]);
        ...
        const otherEmpRes = await (0, db_1.query)('SELECT id, full_name FROM employees WHERE id NOT IN ($1, $2) AND is_active = TRUE AND joining_date <= $3 LIMIT 1', [emp1, futureEmpId, testDate]);
        ...
        const absentEmployeesRes = await (0, db_1.query)(`SELECT e.id, e.full_name 
       FROM employees e
       WHERE e.is_active = TRUE 
        ...
        for (const emp of absentEmployeesRes.rows) {
            await (0, db_1.query)(`INSERT INTO attendance (employee_id, date, time, status, remarks, source, is_locked)
         VALUES ($1, $2, '00:00:00', 'ABSENT', 'Automatically marked ABSENT at cutoff', 'SYSTEM_AUTO_ABSENT', TRUE)
         ON CONFLICT (employee_id, date) DO NOTHING`, [emp.id, testDate]);
            autoAbsentCount++;
        }
       ...
       WHERE date = $1 AND is_locked = FALSE`, [testDate]);
        console.log(`Locked ${lockRes.rowCount} existing records.`);
        // Check results in the database
        const emp1RecordRes = await (0, db_1.query)('SELECT status, is_locked, check_out_time FROM attendance WHERE employee_id = $1 AND date = $2', [emp1, testDate]);
        ...
        if (otherEmp) {
            const otherRecordRes = await (0, db_1.query)('SELECT status, is_locked, source FROM attendance WHERE employee_id = $1 AND date = $2', [otherEmp.id, testDate]);
        ...
        const secondAbsentRes = await (0, db_1.query)(`SELECT e.id 
       FROM employees e
       WHERE e.is_active = TRUE 
         AND (e.is_deleted = FALSE OR e.is_deleted IS NULL)
         AND e.joining_date <= $1
        ...
        const secondLockRes = await (0, db_1.query)(`UPDATE attendance 
       SET is_locked = TRUE, updated_at = NOW() 
       WHERE date = $1 AND is_locked = FALSE`, [testDate]);
        ...
        await (0, db_1.query)('DELETE FROM attendance WHERE date = $1', [testDate]);
        await (0, db_1.query)('DELETE FROM employees WHERE id = $1', [futureEmpId]);
