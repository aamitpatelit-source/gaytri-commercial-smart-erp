...
async function verifyPasswords() {
    const res = await (0, db_1.query)('SELECT email, password_hash FROM admins');
    console.log('\n--- BCRYPT PASSWORD VERIFICATION ---');
    for (const row of res.rows) {
        const isDefault = bcryptjs_1.default.compareSync('workforce@2026', row.password_hash);
        const isUserPass = bcryptjs_1.default.compareSync('amit8340', row.password_hash);
        console.log(`Email: ${row.email}`);
        console.log(`  Matches "workforce@2026": ${isDefault}`);
        console.log(`  Matches "amit8340": ${isUserPass}`);
    }
}
async function runFlowForUser(baseUrl, email, password) {
    console.log(`\n==================================================`);
    ...
    if (markRes.status === 200) {
        ...
        if (markData.success) {
            ...
            const historyRes = await makeRequest(`${baseUrl}/attendance/history?start_date=${todayStr}&end_date=${todayStr}`, 'GET', headers, null);
