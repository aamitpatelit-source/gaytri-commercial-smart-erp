...
async function cleanAndSeedProduction() {
    ...
    try {
      ...
      `, [item.seq]);
        ...
        const passwordHash = await bcryptjs_1.default.hash(superAdminPassword, 10);
    ...
    `, [superAdminId, superAdminEmail, passwordHash]);
        ...
        if (results['Employees'] !== 0)
            throw new Error('Verification Failed: Employees count is not 0!');
        if (results['Attendance Records'] !== 0)
            throw new Error('Verification Failed: Attendance Records count is not 0!');
        if (results['Payroll Records'] !== 0)
            throw new Error('Verification Failed: Payroll Records count is not 0!');
        if (results['Leave Requests'] !== 0)
            throw new Error('Verification Failed: Leave Requests count is not 0!');
        if (results['Manager Accounts'] !== 0)
            throw new Error('Verification Failed: Manager Accounts count is not 0!');
        if (results['Super Admin Accounts'] !== 1)
            throw new Error('Verification Failed: Super Admin Accounts count is not 1!');
        if (results['Total Admin Accounts'] !== 1)
            throw new Error('Verification Failed: Total Admin Accounts count is not 1!');
        console.log('\n=====================================================');
