...
async function runVerification() {
    ...
    try {
        ...
        const empRes = await client.query(`INSERT INTO employees (employee_id, full_name, mobile, joining_date, salary_type, monthly_salary, role, is_active)
       VALUES ($1, 'QA Verification Employee', '9999999999', NOW(), 'MONTHLY', 26000.00, 'EMPLOYEE', TRUE)
       RETURNING id, employee_id, full_name, monthly_salary`, [tempEmpId]);
        ...
        const checkInEval = (0, calculationService_1.evaluateCheckIn)(checkInTime, settings);
        ...
        const insRes = await client.query(`INSERT INTO attendance (employee_id, date, time, check_in_time, status, remarks, source)
       VALUES ($1, $2, $3, $3, $4, 'QA Test Check-In', 'QA_TEST')
       RETURNING id, status, check_in_time, check_out_time`, [emp.id, testDate, checkInTime, checkInEval.status]);
        ...
        const checkOutEval = (0, calculationService_1.evaluateCheckOut)(checkInTime, checkOutTime, existingIsLate, checkInEval.lateMinutes, settings);
        ...
        await client.query(`UPDATE attendance SET check_out_time = $1, status = $2, updated_at = NOW() WHERE id = $3`, [checkOutTime, checkOutEval.status, dbRecord1.id]);
